...
bool equalNodes(struct cmd* node1, struct cmd* node2){
    ...
    switch(node1->type){
        case CONSEC:
            ...
            return equalNodes(consecNode1->left, consecNode2->left) 
                    && equalNodes(consecNode1->right, consecNode2->right);

        case PIPE:
            struct pipe_cmd* pipeNode1 = (struct pipe_cmd*) node1;
            struct pipe_cmd* pipeNode2 = (struct pipe_cmd*) node2;
            return equalNodes(pipeNode1->left, pipeNode2->left) 
                    && equalNodes(pipeNode1->right, pipeNode2->right);
        ...
        case REDIR:
            ...
            return equalNodes(redirectNode1->command, redirectNode2->command)
                    && strcmp(redirectNode1->file, redirectNode2->file)
                    && redirectNode1->mode == redirectNode2->mode
                    && redirectNode1->fd == redirectNode2->fd;

        case EXEC:
            struct exec_cmd* execNode1 = (struct exec_cmd*) node1;
            struct exec_cmd* execNode2 = (struct exec_cmd*) node2;

            if (execNode1->size!=execNode2->size) return false;

            int size = execNode1->size;

            for(int i=0; i<size; i++){
                if(!strcmp(execNode1->eargv[i], execNode2->eargv[i])) return false;
            }
